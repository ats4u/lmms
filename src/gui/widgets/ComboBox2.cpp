...
void ComboBox2::mouseMoveEvent( QMouseEvent* event )
{
	// printf( "void ComboBox2::mouseMoveEvent( QMouseEvent* event )\n" );
	QPoint currPos = event->pos();
	int diffY = currPos.y() - m_lastMousePos.y();
	// printf( "%d\n",  diffY );
	if ( 0 != diffY )
		m_moved = true;

	model()->setInitValue( m_lastValue + ( diffY / 16 ) );
}

void ComboBox2::mouseReleaseEvent( QMouseEvent* event )
{
	if( model() == NULL )
	{
		return;
...
// 	}
 	...
 	{
 		p.setFont( font() );
 		p.setClipRect( QRect( 4, 2, width() - 8, height() - 2 ) );
 		QPixmap pm = model()->currentHasPixmap() ?  model()->currentData().pixmap() : QPixmap();
 		int tx = 5;
 		if( ! pm.isNull() )
 		{
